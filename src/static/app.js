      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantItems = participants.length
          ? participants.map((email) => `
              <li class="participant-row" data-email="${email}" data-activity="${name}">
                <span>${email}</span>
                <button type="button" class="delete-participant" aria-label="Remove ${email}" title="Remove ${email}">×</button>
              </li>`).join("")
